...
from util import count_parameters, _ECELoss, psd_safe_eigen, psd_safe_cholesky, find_module_by_name
...
def parse_args():
    ...
    parser.add_argument('--arch', type=str, default='deq')
...
def main():
    ...
    train_loader_noaug, val_loader, test_loader = data_loaders(args,
                                                               valid_size=0.2 if 'cifar' in args.dataset else 0.01,
                                                               noaug=True)
...
#     torch.save(g_subsample, "g.pt")
#     torch.save(x_subsample, "x.pt")

#     g_subsample = torch.load("g.pt")
#     x_subsample = torch.load("x.pt")
#     print(x_subsample.shape)
#     print(g_subsample.shape)
...
#     print(torch.dist(Psi(x_subsample).flatten(0, 1) @ Psi(x_subsample).flatten(0, 1).T, K.to(device)), torch.dist(K, torch.zeros_like(K)))
    ...
    with torch.no_grad():
        ...
        for i, (x, y) in tqdm(enumerate(train_loader_noaug), desc='Passing the training set',
                              total=len(train_loader_noaug)):
            ...
            if args.search_freq and (i + 1) * args.batch_size % args.search_freq == 0:
                ...
                val_loss, val_acc, val_ece = lla_test(val_loader, model, device, args, Psi, cov_inv, verbose=True)
...
def batch_grad(model, x_batch, y_batch, args):
    ...
    g_batch = torch.zeros(x_batch.shape[0] * (1 if args.only_target_jacobian else args.num_classes),
...
def lla_test(test_loader, model, device, args, Psi, cov_inv, verbose=True):
    ...
    with torch.no_grad():
        for x, y in tqdm(test_loader, desc='Testing', total=len(test_loader)):
            ...
            F_samples = (psd_safe_cholesky(F_var) @ torch.randn(F_var.shape[0], F_var.shape[1], args.num_samples_eval,
